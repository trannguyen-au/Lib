...
$.fn.wnGallery = function(options) {
  ...
  function ClosePopupGallery() {
    ...
    var currentImageHolder = $(imgHolderList[index]);//$("#"+cId+"-thumbnail-wrap-inner").find("div.img-holder[rel='"+index+"']");
    currentImageHolder.addClass("active");
    
    // copy the current image to the mainImage
    if($("#"+cId+"-wnGallery-popup-main-image").find("img").size()==0) {
      //$("#"+cId+"-wnGallery-popup-main-image").append("<img>");
      //mainImage = $("#"+cId+"-wnGallery-popup-main-image img");
    }
	  var currentImage = currentImageHolder.children("img");//$(imageList.get(index));
