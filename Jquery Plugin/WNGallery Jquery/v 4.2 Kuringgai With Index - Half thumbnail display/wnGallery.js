...
$.fn.wnGallery = function(options) {
  ...
  function ClosePopupGallery() {
    ...
    $("<img alt='"+title+"'/>") // Make in memory copy of image to avoid css issues
        ...
        var clientHeight = $(newImg).height();
        var calcHeight = this.height*$("#"+cId+"-wnGallery-popup-main-image").width()/this.width;
        $(newImg).hide();
			  $("#"+cId+"-wnGallery-popup-main-image").html(newImg);
        $("#"+cId+"-wnGallery-popup-main-image").animate({height:clientHeight==0||true?calcHeight:clientHeight}, function() {
          $(newImg).delay(100).fadeIn(300, function() {
  ...
  var isHalfDisplayAtEnd = true;
	
	function MoveViewPort(imageIndex, direction) {
    var offset = !isHalfDisplayAtEnd ? (options.pagerImageSize/2) : 0;
      ...
      if(imageIndex == startIndex) {
        isHalfDisplayAtEnd = false;
        mainPager.animate({'left':-((options.pagerImageSize)*currentDisplayedIndex * numberOfDisplayedImage)} );
      }
      else if(imageIndex < startIndex) {  // need to move left
        currentDisplayedIndex--;
        mainPager.animate({'left':-((options.pagerImageSize)*currentDisplayedIndex * numberOfDisplayedImage + offset)} );
      }
      ...
      if(imageIndex == endIndex-1)  {
        isHalfDisplayAtEnd = false;
        mainPager.animate({'left':-((options.pagerImageSize)*currentDisplayedIndex * numberOfDisplayedImage+(options.pagerImageSize/2))});
      }
      else if(imageIndex >= endIndex) { // need to move right
        currentDisplayedIndex++;
        mainPager.animate({'left':-((options.pagerImageSize)*currentDisplayedIndex * numberOfDisplayedImage)});
      }
