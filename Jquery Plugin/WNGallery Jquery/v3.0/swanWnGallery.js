...
*  @version 2.1  16/05/2012
  - add navigation on top right of the image
...
$.fn.wnGallery = function(options) {
    ...
    canvasSize: {width:392, height:298},
  ...
  var galleryNavigation = "<div id='"+cId+"-nav' style='position:absolute;top:15px;right:15px;z-index:9999'><div style='position:relative'><a class='ui-icon ui-icon-triangle-1-w img-gallery-pager' rel='pre' href='#' style='margin-right:3px'></a><a class='ui-icon ui-icon-triangle-1-e img-gallery-pager' rel='next' href='#'></a></div></div>";
    container.before("<div style='width: 406px;height: 310px; position:relative'>"+galleryNavigation+"<table style='border-collapse:collapse; width:"+options.canvasSize.width+"px;height:"+options.canvasSize.height+"px; margin-bottom:5px;' id='"+ cId +"_table' class='showarea'><tr><td style='width:"+options.canvasSize.width+"px;height:"+options.canvasSize.height+"px;'></td></tr></table></div>");
  ...
  $("#"+ cId +"_table td").append()
    ...
    if(imageList.length>0) {
      ...
      $("#"+cId+"_table td").html("<img src='"+src+"?w="+options.canvasSize.width+"&h="+options.canvasSize.height+"' style='width:"+options.canvasSize.width+"px;height:292px' alt='"+alt+"' title='"+title+"'/>");
    }
  ...
  $("a.img-gallery-pager").click(function(event){
    if(event==null) event= window.event;
      event.preventDefault();
  ...
  function MovePreImage() {
    ClearTimer();
  
    if(activeIndex==0) {
      activeIndex = imageList.size()-1; // set active to the last image
    }
    else {
      activeIndex--;
    }
